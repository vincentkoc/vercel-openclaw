...
import { prepareHostSleep, runRemoteTurn } from './remote-turn.mjs';
...
async function main() {
  ...
  const credentials = { token: process.env.VERCEL_OIDC_TOKEN, projectId: manifest.config.projectId, teamId: manifest.config.teamId };
  const profile = parseProfile(cfg.cloudWorkers.profiles.vercel.settings);
  const result = await runRemoteTurn({ ...input, base, rpc, observe,
    ...(native ? nativeSlackController({ input: native, path: process.env.OPENCLAW_HOST_ADMISSION_PATH, secret: process.env.SLACK_SIGNING_SECRET, rpc, observe }) : {}),
    connectOperator: sessionKey => connectTestOperator({ url: 'ws://127.0.0.1:18789', token, sessionKey, localApprovals: Boolean(native) }),
    ...
    workerFor: async placement => {
      ...
      const identity = await box.runCommand({ cmd: 'node', args: ['--input-type=module', '-e', 'import {DatabaseSync} from "node:sqlite";import {homedir} from "node:os";import {join} from "node:path";const db=new DatabaseSync(join(homedir(),".openclaw-vercel-worker/state/state/openclaw.sqlite"),{readOnly:true});try{process.stdout.write(db.prepare("SELECT device_id FROM device_identities WHERE identity_key = ?").get("primary").device_id)}finally{db.close()}'], timeoutMs: 5000, signal: AbortSignal.timeout(10_000) });
      assert.equal(identity.exitCode, 0);
      assert.equal(nodes[0].nodeId, await identity.stdout(), 'Remote node does not belong to the allocated VM');
      return { name, nodeId: nodes[0].nodeId, assertStopped: async () => assert.equal((await Sandbox.get({ ...credentials, name, resume: false })).status, 'stopped') };
    },
  });
  assert.equal(hash(readFileSync(configPath)), configDigest, 'Gateway configuration changed during the turn');
  observe('worker-reclaimed');
  const suspensionChecks = [];
  const suspension = await prepareHostSleep({ rpc, requestId: `host-${input.eventId}`, observe: value => {
    suspensionChecks.push({ at: Date.now(), ...value });
    writeFileSync(`${base}/host-suspension.json`, JSON.stringify(suspensionChecks));
  } });
  await stopGateway();
  observe('gateway-stopped');
  await ingress.close();
  ingress = undefined;
  process.stdout.write(`OPENCLAW_HOST_RESULT=${JSON.stringify({ ...result, suspension, phases, gatewayStopped: true })}\n`);
}

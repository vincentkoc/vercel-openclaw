...
let token;
try {
  assert(['observe', 'verify'].includes(command), 'Use observe <write|read> or verify');
  ...
  if (command === 'verify') {
    ...
    assert.equal((await owned()).status, 'stopped');
    const runs = verifyNativeSlack({ fixture, observations, logs, workers, runtimeDigest });
    save('receipt.json', { status: 'passed', at: new Date().toISOString(), runtimeDigest, runs, workers,
      inputs: Object.fromEntries(['fixture.json', 'requests.jsonl', ...fixture.cases.map(c => `observe-${c.label}.json`)].map(p => [p, sha256(readFileSync(join(root, p)))])),
      verifierSha256: sha256(readFileSync(new URL('./native-slack-proof.mjs', import.meta.url))),
      notTested: ['OAuth expiry', 'cross-thread memory', 'crash recovery', 'adversarial isolation', 'Slack UI paint time'] });
    console.log('NATIVE_SLACK_TWO_TURN_SLEEP_WAKE_PASS');
    console.log(JSON.stringify(runs.map(({ replyMs }) => ({ replyMs }))));
  } else {
    assert(['write', 'read'].includes(label));
    assert(fixture.cases.some(c => c.label === label && c.role === label));
    ...
    assert.equal((await owned()).status, 'stopped', 'Start with VM1 asleep');
    ...
    console.log(nativeSlackPrompt(fixture, label));
    let seen = false;
    ...
    assert(result.snapshot, 'Timed out without current-session snapshot; inspect host logs, do not assume cleanup');
